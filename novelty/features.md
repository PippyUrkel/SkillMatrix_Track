...
- **AI Helper Panel:** In-context AI assistant available on the learning page.
...
| 2 | **Interest Profiling** | ❌ Not Started | Questionnaire or AI-based interest discovery (e.g., Holland Codes / RIASEC model) to align career suggestions with personal interests. |
| 3 | **Career Path Suggestion Engine** | ❌ Not Started | Based on combined skills + academics + interests, recommend specific career paths with reasoning (not just job listings). |
| 4 | **Paid Course Recommendations** | 🟡 Partial | Currently only generates free YouTube-based courses. Need integration with Coursera, Udemy, edX APIs for paid course suggestions with pricing. |
| 5 | **Student Profile vs. Industry Comparison Dashboard** | 🟡 Partial | Skill gap page exists but needs a side-by-side visual comparing student's full profile against industry benchmarks. |
...
| 8 | **LinkedIn Skill Import** | ❌ Not Started | Pull skills directly from LinkedIn profile using the LinkedIn ID captured during onboarding. |
